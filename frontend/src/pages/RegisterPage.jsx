...
import { Mail, Lock, Loader2, User, Link as LinkIcon } from 'lucide-react';
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] p-4 bg-[#fcfcfd] dark:bg-black relative overflow-hidden">
      ...
      <motion.div 
        ...
        className="relative z-10 w-full max-w-[390px] bg-white dark:bg-gray-900/50 backdrop-blur-xl p-6 sm:p-8 rounded-[2rem] shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-gray-800"
      >
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Join LumiLink to manage your links</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <label className="block text-[11px] font-bold text-slate-900 dark:text-white ml-1 uppercase tracking-wider">Full Name</label>
            <div className="relative group">
              ...
              <input
                type="text"
                required
                className="w-full pl-10 pr-4 py-2 bg-slate-50/50 dark:bg-black/50 border border-slate-200 dark:border-gray-800 rounded-xl focus:bg-white dark:focus:bg-black focus:border-slate-900 dark:focus:border-white focus:outline-none transition-all placeholder:text-slate-400 text-xs"
        ...
        <p className="text-center mt-6 text-xs text-slate-500 dark:text-slate-400 font-medium">
          ...
          <Link to="/login" className="text-slate-900 dark:text-white font-bold hover:underline underline-offset-4">
            Sign in
          </Link>
        </p>
      </motion.div>
